 ...
 * <para>
 * <programlisting>
 * gst-launch filesrc location=test.mp3 ! mad ! audioconvert ! fftwspectrum !
 *moodbar height=50 ! pngenc ! filesink location=test.png
 * </programlisting>
 * </para>
 * </refsect2>
 */
...
enum { ARG_0, ARG_HEIGHT, ARG_MAX_WIDTH };

static GstStaticPadTemplate sink_factory = GST_STATIC_PAD_TEMPLATE(
    "sink", GST_PAD_SINK, GST_PAD_ALWAYS, GST_STATIC_CAPS(SPECTRUM_FREQ_CAPS));
...
static gboolean gst_moodbar_sink_event(GstPad* pad, GstObject* parent,
                                       GstEvent* event);

static GstFlowReturn gst_moodbar_chain(GstPad* pad, GstObject* object,
                                       GstBuffer* buf);
...
static void gst_moodbar_class_init(GstMoodbarClass* klass) {
  ...
  g_object_class_install_property(
      gobject_class, ARG_HEIGHT,
      g_param_spec_int("height", "Image height",
                       "The height of the resulting raw image", 1, G_MAXINT32,
                       HEIGHT_DEFAULT, G_PARAM_READWRITE));

  g_object_class_install_property(
      gobject_class, ARG_MAX_WIDTH,
      g_param_spec_int(
          "max-width", "Image maximum width",
          "The maximum width of the resulting raw image, or 0 for no rescaling",
          0, G_MAXINT32, MAX_WIDTH_DEFAULT, G_PARAM_READWRITE));

  gstelement_class->change_state = GST_DEBUG_FUNCPTR(gst_moodbar_change_state);
  ...
  gst_element_class_set_static_metadata(
      gstelement_class, "Moodbar analyzer", "Filter/Converter/Moodbar",
      "Convert a spectrum into a stream of (uchar) rgb triples representing "
      "its \"mood\"",
 ...
 */
static void gst_moodbar_init(GstMoodbar* mood) {
  GstElementClass* klass = GST_ELEMENT_GET_CLASS(mood);

  mood->sinkpad = gst_pad_new_from_template(
  ...
  mood->srcpad = gst_pad_new_from_template(
...
static void calc_barkband_table(GstMoodbar* mood) {
  guint i;
  guint barkband = 0;

  /* Avoid divide-by-zero */
  if (!mood->size || !mood->rate) return;

  if (mood->barkband_table) g_free(mood->barkband_table);

  mood->barkband_table = g_malloc(NUMFREQS(mood) * sizeof(guint));

  for (i = 0; i < NUMFREQS(mood); ++i) {
    if (barkband < 23 && (guint)GST_SPECTRUM_BAND_FREQ(
                             i, mood->size, mood->rate) >= bark_bands[barkband])
      barkband++;
...
static GstStateChangeReturn gst_moodbar_change_state(
 ...
 */
static gboolean allocate_another_frame(GstMoodbar* mood) {
  mood->numframes++;

  /* Failsafe */
  if (mood->numframes == MAX_TRIPLES) return FALSE;
  ...
  if (mood->numframes % FRAME_CHUNK == 0) {
    ...
    if (mood->r == NULL || mood->g == NULL || mood->b == NULL) return FALSE;
  }
 ...
 */
static GstFlowReturn gst_moodbar_chain(GstPad* pad, GstObject* parent,
                                       GstBuffer* buf) {
  GstMoodbar* mood = GST_MOODBAR(parent);
  guint i;
  ...
  if (!allocate_another_frame(mood)) return GST_FLOW_ERROR;

  /* Calculate total amplitudes for the different bark bands */

  for (i = 0; i < 24; ++i) amplitudes[i] = 0.f;
  ...
  for (i = 0; i < 24; ++i) rgb[i / 8] += amplitudes[i] * amplitudes[i];
 ...
 */
static void normalize(gdouble* vals, guint numvals) {
  ...
  for (i = 0; i < numvals; i++)
    vals[i] =
        isfinite(vals[i]) ? MIN(1.f, MAX(0.f, (vals[i] - mini) / delta)) : 0.f;
}
...
static void gst_moodbar_finish(GstMoodbar* mood) {
  ...
  if (mood->max_width == 0 || mood->numframes <= mood->max_width)
  ...
  buf = gst_buffer_new_and_alloc(output_width * mood->height * 3 *
                                 sizeof(guchar));
  if (!buf) return;
